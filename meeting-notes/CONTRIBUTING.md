...
Commenting on an issue is super easy - simply click on the card you want to comment on.



Here you will see the latest discussion on the issue from different members of the working group or community.
You can read through the discussion, and add a comment at the bottom by typing in the text box and selecting Add comment


Opening a new issue
To open a new issue, navigate to the Project board page.
Under each column is an `+ Add item` button. Click the one under the `Ideas` Column


You’ll be prompted to choose the repository you want to open the issue under. Type `#` and select `community-management`



Next you’ll need to open a new issue. Select `Create new issue`.



You’ll be prompted to choose an issue template - we have designed the `General Issue` template to contain the important information you’ll need. Select ` 🔧 General Issue`.


Now you’ll need to fill in the details about the issue you are creating. This includes:
...
Once you have added all the details, select `Create Issue`


Congrats! Your issue will now appear in the `Ideas` column of the project board.
...
Assign yourself to it using the `Assignees` function
Add any updates in comments in the issue
Move it into the relevant columns. For instance, if you have started work on it, you should move the issue to the `In progress column.

Contributing through our Google Form
You’re also able to contribute to our project board through this Google Form.
...
This form will come through to the Community Management Working Group, who will review it. If approved, it will be added an an issue in the `Ideas` column of our project board. If they have any further questions, they’ll reach out to you.
...
Anybody raises any pressing issues that need immediate discussion with the group
The group gets progress reports on all `Blocked` and `In Progress` tasks, and decides next steps as required
The group talks through `Ideas` and either approves or rejects them (providing reasons, if approved they move it into `Backlog`, if rejected the reasons communicated with the person who opened the issue.
The group reviews `Planned` and moves items into `In progress`, ‘Backlog` or keeps them in `Planned` as required.
The group reviews the `Backlog` and moves anything planned to be started in the upcoming week into `Planned`.
AOB.
♻️ License
...
These guidelines were adapted from the Alan Turing Institute's Reproducible Project Template. 
