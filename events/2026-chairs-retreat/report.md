# UK TRE Community Review & Report
## Executive Summary

In May 2026 the co-chairs of the UK Trusted Research Environment (UK TRE) community met for a strategic retreat to consider the progress of the community, the challenges lying ahead and agree on priorities for the future. The discussions reflect a community that has accomplished a great deal in a very short period of time. The UK TRE conference has grown into a recognised and valued event bringing together technical specialists, governance professionals, legal experts, public contributors, researchers and operational teams from across the sector. This is a big step in a fragmented and fast-moving landscape to create a space where these groups can connect regularly and learn from each other. 

The retreat also highlighted a number of challenges that cannot be ignored. The wider TRE ecosystem is growing more complex, with many groups working on similar issues but with little coordination. A lot of the community’s work still depends on volunteer effort, which brings risks around sustainability and capacity. There is also an increasing need to ensure that the community’s activities remain focused on areas where it can add the most value. A key theme from the retreat was that the UK TRE Community should focus on two core roles. First, to sustain the delivery and development of the annual conference as an important national convening event. Second, a connector and coordinator in the wider TRE ecosystem, helping organisations and initiatives work better together. Wherever possible, other activities should be delivered in collaboration rather than in parallel.

## Background

TRE is gaining recognition as the standard approach to secure access of sensitive data in research, health, government and public sector settings. The UK TRE Community was set up to provide a collaborative forum for those who design, govern, operate and use TREs. As the field has grown, so has the need to think about how the community grows and where it can have the most impact. The May 2026 retreat was an opportunity for community leaders and contributors to step away from their day-to-day work and reflect on a number of key questions:

1.  What has the community accomplished to date?
2.  What activities generate the most value?
3.  What are the biggest risks and challenges?
4.  What would a model of a sustainable future look like?

The discussions considered both the current state of the community and the wider policy, governance, and technology landscape in which TREs operate.

## Key Achievements

### Building a National Community

One of the major successes of the community has been to bring together people who would otherwise be working in isolation. The annual conference has become a forum for the sector, and has been instrumental in generating new collaborations, working groups, partnerships and funded initiatives. Co-chairs highlight the significance of peer-to-peer engagement and the chance to share experiences and learn from organisations facing similar challenges. Those working in and around TREs have also developed a greater sense of professional identity which helps to build connections across institutions and disciplines . This progress is encouraging, but there is also value in understanding how far the community reaches beyond its existing networks and how it can continue to bring new organisations and perspectives into the fold.

### Promoting Open Collaboration

The retreat reinforced the community's commitment to openness, shared learning, and collaborative development. There remains strong support for open-source approaches, reusable outputs, and the principle that publicly funded work should create resources that can be shared and reused across the wider sector. This commitment continues to be one of the community's defining strengths.

## Strategic Priorities

### Supporting the Normalisation of TREs

There was agreement that TREs should become a routine and trusted part of conducting sensitive data research. Improving researcher experience, reducing unnecessary barriers, and increasing awareness among data custodians and decision-makers were all identified as important objectives. However, co-chairs also recognised that achieving this ambition will require action from a wide range of stakeholders, including funders, regulators, institutions, and policymakers. The community's role is therefore best understood as one of influence, coordination, and advocacy rather than direct delivery.

### Encouraging Open and Interoperable Approaches

The community remains well placed to support conversations around standards, interoperability, and reusable infrastructure. By helping organisations learn from one another and share common approaches, the community can contribute to reducing duplication and lowering barriers for newer or smaller organisations entering the field.

### Strengthening Collaboration Across the Ecosystem

A recurring theme throughout the retreat was the need for greater coordination across the growing number of groups operating within the TRE and wider data access landscape. While there is strong goodwill and a shared desire to collaborate, co-chairs recognised that overlapping initiatives and duplicated effort remain common. The opportunity for the community is to help improve connectivity, communication, and alignment between groups without seeking to control or replace existing initiatives.

### Supporting Workforce Development

The TRE workforce continues to grow, but career pathways, role definitions, and competency frameworks remain inconsistent across organisations. Co-chairs recognised the importance of supporting professional development in this area. However, there was also a clear view that the community should work alongside existing initiatives rather than creating additional parallel programmes.

### Governance, Regulation, and Public Trust

### Public Trust

The discussions during the retreat highlighted the importance of ensuring that public engagement moves beyond awareness-raising and supports meaningful involvement in discussions about how sensitive data is accessed, governed, and used.

Building trust requires transparency, accountability, and clear evidence that public perspectives are informing decisions.

### Legal and Regulatory Change

The regulatory environment surrounding data access and research continues to evolve rapidly.

Developments relating to data protection, artificial intelligence, cyber security, and data-sharing frameworks create both opportunities and challenges for organisations operating TREs. Co-chairs agreed that the community can provide value by supporting horizon scanning, sharing guidance, and creating opportunities for collective learning and discussion.

### Transparency

There was strong support for increasing transparency around data use, governance arrangements, and research activity. Transparency not only supports public confidence but also strengthens accountability and helps demonstrate the value that secure data access can deliver.

### Key Risks

### Fragmentation

Fragmentation was identified as the most significant strategic risk facing the wider ecosystem. The growing number of groups, initiatives, and networks working on related issues creates a risk of duplication, reduced visibility, and missed opportunities for collaboration. Improving coordination across the landscape is therefore one of the areas where the community could potentially add the greatest value.

### Sustainability

Much of the community's success has been made possible through the dedication and goodwill of volunteers. While this has enabled significant progress, it is unlikely to provide a sustainable long-term model. Without additional capacity and support, there is a risk that key activities become difficult to maintain and that community leaders face increasing pressure on their time.

### Maintaining Momentum

As with many collaborative networks, there is a risk that valuable discussions do not always translate into sustained action. Co-chairs recognised the importance of developing practical mechanisms that help turn ideas into deliverable outcomes while recognising the capacity constraints faced by many contributors.

**Future Direction**

### Continuing to Grow the Conference

The annual conference should remain the community's flagship activity and primary convening event. Future development should focus on broadening co-chairs, increasing representation from policy and public involvement communities, and ensuring the programme continues to evolve alongside the needs of the sector.

### Strengthening Coordination

The retreat identified coordination as one of the most significant opportunities for future development. Practical activities such as shared communications, coordinated calendars, improved onboarding, and stronger links between existing initiatives could help reduce duplication and improve visibility across the ecosystem. The aim is not to create a central authority, but to improve connectivity and collaboration between independent groups with shared interests.

### Recommendations

The retreat concluded with six recommendations:

1.  Clarify and formalise the UK TRE Community's strategic purpose around conference delivery and ecosystem coordination.
2.  Develop a coordination framework that improves visibility, communication, and collaboration across existing initiatives.
3.  Undertake a wider stakeholder engagement exercise between July and September 2026 to test and refine these conclusions.
4.  Pilot practical coordination activities, including shared communications, calendars, onboarding processes, and collaboration pathways.
5.  Develop a sustainability plan that addresses the community's reliance on volunteer capacity and explores options for future support.
6.  Continue promoting open, collaborative, and interoperable approaches as core principles across all community activities.

### Assessment

Following review of the UK TRE Community's activities, governance arrangements, strategic objectives, sustainability planning, and proposed delivery model, the overall assessment is:

|     |     |     |
| --- | --- | --- |
| **Area** | **Rating** | **Commentary** |
| Strategic Alignment | 🟢 Green | Strong alignment with DARE UK objectives and wider national TRE priorities. |
| Community Engagement | 🟢 Green | Evidence of sustained and growing engagement across multiple stakeholder groups. |
| Governance | 🟢 Green | Mature governance culture with clear values around openness, collaboration and accountability. |
| Delivery Capability | 🟢 Green | Highly experienced leadership team with proven delivery track record. |
| Sustainability | 🟠 Amber | Long-term funding and leadership succession remain significant challenges. |
| Stakeholder Coordination | 🟠 Amber | Fragmentation across the wider ecosystem continues to impact efficiency. |
| Resource Capacity | 🟠 Amber | Heavy reliance on volunteer effort presents operational risks. |
| Overall Programme Confidence | 🟢 Green/Amber | Strong likelihood of successful delivery provided sustainability and coordination risks are actively managed. |

### Strategic Risk Register

|     |     |     |     |     |     |
| --- | --- | --- | --- | --- | --- |
| **ID** | **Risk** | **Likelihood** | **Impact** | **Rating** | **Mitigation** |
| R1  | Loss of key volunteer leaders | High | High | 🔴 Red | Establish leadership pathways and succession planning. |
| R2  | Insufficient funding beyond current programme | High | High | 🔴 Red | Develop diversified funding model and sponsorship strategy. |
| R3  | Fragmentation of TRE ecosystem | High | Medium | 🟠 Amber | Create shared calendar, communications and coordination mechanisms. |
| R4  | Community engagement fatigue | Medium | Medium | 🟠 Amber | Focus on high-value activities and avoid duplication. |
| R5  | Lack of consensus on future direction | Medium | Medium | 🟠 Amber | Structured community consultation and transparent decision-making. |
| R6  | Overreliance on annual conference | Medium | Medium | 🟠 Amber | Introduce complementary engagement mechanisms throughout the year. |
| R7  | Stakeholder duplication with other groups | Medium | Medium | 🟠 Amber | Formal collaboration agreements and clearer role definition. |
| R8  | Reduced external funding landscape | Medium | High | 🟠 Amber | Develop scalable operating models. |
| R9  | Public trust challenges arising from wider data incidents | Low | High | 🟠 Amber | Continue transparency and public engagement activities. |
| R10 | Failure to demonstrate measurable impact | Medium | Medium | 🟠 Amber | Develop KPIs and community impact metrics. |

**Stakeholder Analysis**

**Next month meeting - agenda July 2026**

**High Influence / High Interest**

**Core Delivery Partners**

**Member list from Slack & Jisc (Cass & Simon) - KUMU**

- DARE UK
- UK TRE Community Management Working Group (CMWG)
- Research Community Manager (RCM Coop)
- Alan Turing Institute - tricky one / low interest as UKTRE - no priorities for them - high influence & low interest
- Manchester University NHS Foundation Trust
- King's College London - individual / high influence & high interest

**Engagement Strategy**

- Monthly engagement
- Strategic planning involvement
- Shared decision-making

**High Influence / Medium Interest**

**National Stakeholders**

- NHS England
- Health Data Research UK
- UK Research and Innovation (UKRI)
- Office for National Statistics
- National Data Guardian
- Information Commissioner's Office

**Engagement Strategy**

- Quarterly strategic updates
- Consultation opportunities
- Policy engagement

**Medium Influence / High Interest**

**Operational Community**

- TRE Operators
- Information Governance Professionals
- Research Software Engineers
- Data Stewards
- Security Specialists
- Public Contributors and PPIE Groups

**Engagement Strategy**

- Community events
- Working groups
- Conference participation
- Collaborative projects

**Emerging Stakeholders**

**Future Strategic Partners**

- Welcome Trust
- Medical Research Charities
- International TRE Networks
- European Federated Data Initiatives
- National Cyber Security Centre
- Academic Training Programmes

**Engagement Strategy**

- Targeted outreach
- Strategic partnerships
- Conference representation

**Delivery Roadmap (June 2026 - March 2027)**

**Phase 1: Consolidation**

**June 2026**

Objectives:

- Consolidate retreat outputs
- Produce strategic review report
- Establish governance for consultation phase

Deliverables:

- Final strategic review
- Initial risk register
- Stakeholder engagement plan

Success Measure:

- Report approved by CMWG

**Phase 2: Community Validation**

**July - September 2026**

Objectives:

- Validate findings with wider community
- Gather stakeholder feedback
- Refine strategic priorities

Deliverables:

- Community consultation
- Survey findings
- Updated strategic framework

Success Measure:

- Representation from all major stakeholder groups

**Phase 3: Strategy Launch**

**October 2026**

Objectives:

- Publish refreshed UK TRE strategy
- Establish agreed future operating model

Deliverables:

- Strategic Plan 2026-2030
- Community Charter
- Coordination Framework

Success Measure:

- Formal community endorsement

**Phase 4: Pilot Interventions**

**November 2026 - January 2027**

Objectives:

- Test practical coordination initiatives

Deliverables:

- Shared events calendar
- Community onboarding framework
- Cross-community communications process
- Leadership pathway framework

Success Measure:

- Increased engagement and reduced duplication

**Phase 5: Sustainability Planning**

**January - March 2027**

Objectives:

- Establish long-term sustainability model

Deliverables:

- Funding strategy
- Leadership succession plan
- Operating model to 2030

Success Measure:

- Sustainable funding options identified
- Leadership pipeline established

**Recommended Key Performance Indicators (KPIs) - further work to develop (mountain of engagement)**

**Community Growth**

- Membership growth rate
- Event attendance but also Slack/mailing list/conversation
- Active participation levels

**Collaboration**

- Number of cross-organisational collaborations
- Joint funding applications
- Shared outputs produced

**Sustainability**

- Leadership succession readiness
- Volunteer retention
- Funding diversification

**Impact**

- Community satisfaction score
- Stakeholder engagement score
- Adoption of shared standards and practices

# UK TRE Community Project Deliverables

## WP1 – Community Mapping

| **Deliverable** | **Description** | **Outputs / Components** | **Milestone** |
| --- | --- | --- | --- |
| **Deliverable 1: UKTRE Community Database** | Unified dataset of community membership and engagement | UKTRE mailing list, Slack, event attendance, DARE UK landscape data, related community information | —   |
| **Deliverable 2: Interactive Public Community Map** | Public-facing map of UKTRE ecosystem | Stakeholders, related communities, relationships and connections (publicly accessible) | Soft launch + full launch (Quarterly meetings) |
| **Deliverable 3: Internal Community Management Map** | Restricted internal version for management use | Community management, strategy development, impact monitoring | —   |
| **Deliverable 4: Community Impact Metrics Framework** | Metrics derived from mapping activity | Engagement, connectivity, community growth, leadership participation | —   |

WP2 – Community Needs Analysis

| **Deliverable** | **Description** | **Outputs / Components** | **Milestone** |
| --- | --- | --- | --- |
| **Deliverable 5: Community Needs Assessment** | Evidence-based analysis of community needs | Interviews, engagement review, stakeholder consultations | Invitation to interviews |
| **Deliverable 6: Strategic Role Definition for UKTRE** | Defines purpose and positioning of UKTRE | What UKTRE should/should not do; ecosystem role | —   |
| **Deliverable 7: Success Measures (KPIs)** | Defined metrics for success | KPIs aligned to strategy | —   |

**Supporting Outputs:**

- Interview programme
- Analysis report
- Stakeholder engagement findings

WP3 – Engagement Strategy

| **Deliverable** | **Description** | **Outputs / Components** | **Milestone** |
| --- | --- | --- | --- |
| **Deliverable 8: UKTRE Engagement Strategy** | Co-created strategy based on WP2 | Priorities, objectives, activities, success measures, evaluation approach | Launch of engagement strategy |
| **Deliverable 9: Engagement Roadmap** | Practical implementation plan | Phased delivery plan of activities | —   |
| **Deliverable 10: Engagement Activities** | Delivery of engagement programme (Jul 2026–Mar 2027) | Events, workshops, forums, working groups | —   |
| **Deliverable 11: Monitoring & Evaluation Reports** | Tracking implementation and impact | Performance against metrics | —   |

WP4 – Sustainability

| **Deliverable** | **Description** | **Outputs / Components** | **Milestone** |
| --- | --- | --- | --- |
| **Deliverable 12: Leadership Pathway Framework** | Routes into leadership roles | Community, working group, public member leadership pathways | Annual meeting (strategy launch) |
| **Deliverable 13: Leadership Pipeline Monitoring System** | Tracks leadership development | Uses WP1 mapping metrics | Quarterly reporting |
| **Deliverable 14: CMWG Retreat** | Funded strategic retreat (£10k) | Leadership sustainability, governance, funding planning | Retreat milestone |
| **Deliverable 15: Sustainable Funding Model** | Funding strategy for UKTRE | Proposed long-term financial model | —   |
| **Deliverable 16: Funding Forecast to April 2030** | Financial sustainability projection | Forecast to 2030 | End of project milestone |

Ongoing Community Operations Deliverables

| **Deliverable** | **Description** | **Outputs / Components** | **Milestone** |
| --- | --- | --- | --- |
| **Deliverable 17: Maintain Community Infrastructure** | Ongoing operations support | JISC mailing list, Slack, Zenodo, website, communications | Continuous |
| **Deliverable 18: Quarterly Community Meetings/REPORTS (financial reports)** | Regular community convening | Planning, delivery, reporting, outputs publication | Quarterly |
| **Deliverable 19: Annual Community Meeting** | Major annual event | Organisation, delivery, recordings, materials | Annual |
| **Deliverable 20: Public Accessibility Outputs** | Dissemination of outputs | Reports, recordings, Zenodo deposits, communications | Continuous |

Governance & Contractual Deliverables

| **Deliverable** | **Description** | **Outputs / Components** | **Milestone** |
| --- | --- | --- | --- |
| **Deliverable 21: Project Board Operation** | Governance structure in operation | Board meetings, decisions, oversight | Ongoing |
| **Deliverable 22: Progress Reporting** | Formal reporting to funder | Invoicing, milestones, funder reports | Monthly / periodic |
| **Deliverable 23: Financial Reporting** | Financial accountability | Invoices, expenditure reports, final statement | Monthly / end of project |
| **Deliverable 24: Data Protection Documentation** | Compliance with GDPR/data governance | DPIA, data procedures, consent/access arrangements | As required |
| **Deliverable 25: Open Outputs** | Open science compliance | CC BY outputs, open-source licensing (MIT/Apache etc.) | Ongoing |

1\. Programme Overview

| **Work Package** | **Theme** | **Status Focus** | **Core Purpose** |
| --- | --- | --- | --- |
| WP1 | Community Mapping | Data + Infrastructure | Build unified community intelligence system |
| WP2 | Needs Analysis | Insight + Strategy | Define role, needs, and success measures |
| WP3 | Engagement Strategy | Delivery + Activity | Implement engagement programme |
| WP4 | Sustainability | Leadership + Funding | Ensure long-term viability |
| Ops | Community Operations | Ongoing delivery | Maintain infrastructure & engagement |
| Gov | Governance & Reporting | Assurance | Compliance, reporting, oversight |

WP1 – Community Mapping (FOUNDATION LAYER)

| **Deliverable** | **Output** | **Status Indicator** |
| --- | --- | --- |
| UKTRE Community Database | Unified engagement dataset (mailing list, Slack, events, DARE UK data) | ⬜ Not Started / 🟡 In Progress / 🟢 Complete |
| Public Community Map | External stakeholder + relationship map | ⬜ 🟡 🟢 |
| Internal Community Map | Restricted strategic management tool | ⬜ 🟡 🟢 |
| Impact Metrics Framework | Engagement, connectivity, growth, leadership metrics | ⬜ 🟡 🟢 |

**Milestones**

- Soft launch: Quarterly Meeting ⬜/🟡/🟢
- Full launch: Quarterly Meeting ⬜/🟡/🟢

### WP2 – Community Needs Analysis (INSIGHT LAYER)

| **Deliverable** | **Output** | **Status** |
| --- | --- | --- |
| Needs Assessment | Interview + engagement synthesis | ⬜🟡🟢 |
| Strategic Role Definition | UKTRE role clarity (do / don’t / position) | ⬜🟡🟢 |
| Success Measures (KPIs) | Defined performance indicators | ⬜🟡🟢 |

**Supporting Activity**

- Interview programme
- Stakeholder consultation
- Analysis report

**Milestone**

- Interview launch ⬜🟡🟢

WP3 – Engagement Strategy (DELIVERY LAYER)

| **Deliverable** | **Output** | **Status** |
| --- | --- | --- |
| Engagement Strategy | Co-created strategic plan | ⬜🟡🟢 |
| Engagement Roadmap | Delivery plan | ⬜🟡🟢 |
| Engagement Activities | Events, workshops, forums (Jul 2026–Mar 2027) | ⬜🟡🟢 |
| M&E Reports | Tracking against KPIs | ⬜🟡🟢 |

**Milestone**

- Strategy launch ⬜🟡🟢

WP4 – Sustainability (LONG-TERM LAYER)

| **Deliverable** | **Output** | **Status** |
| --- | --- | --- |
| Leadership Framework | Pathways into leadership roles | ⬜🟡🟢 |
| Leadership Monitoring System | Tracking pipeline via WP1 metrics | ⬜🟡🟢 |
| CMWG Retreat | Governance + funding workshop (£10k) | ⬜🟡🟢 |
| Funding Model | Sustainable funding approach | ⬜🟡🟢 |
| Funding Forecast (to 2030) | Financial projection | ⬜🟡🟢 |

**Milestones**

- Strategy + purpose launch ⬜🟡🟢
- Quarterly leadership reporting ⬜🟡🟢
- Funding forecast delivery ⬜🟡🟢

## 3\. Ongoing Operations Dashboard

| **Area** | **Activities** | **Status** |
| --- | --- | --- |
| Infrastructure | JISC, Slack, Zenodo, website | ⬜🟡🟢 |
| Meetings | Quarterly + Annual meetings | ⬜🟡🟢 |
| Communications | Community updates & publishing | ⬜🟡🟢 |
| Public Outputs | Reports, recordings, deposits | ⬜🟡🟢 |

## 4\. Governance & Assurance Dashboard

| **Deliverable** | **Requirement** | **Status** |
| --- | --- | --- |
| Project Board | Oversight + decisions | ⬜🟡🟢 |
| Progress Reporting | Invoicing + funder reporting | ⬜🟡🟢 |
| Financial Reporting | Monthly + final accounts | ⬜🟡🟢 |
| Data Protection | DPIA + governance docs | ⬜🟡🟢 |
| Open Outputs | CC BY + open-source outputs | ⬜🟡🟢 |

5\. Key Risks

| **Risk Area** | **Description** | **Mitigation** |
| --- | --- | --- |
| Governance clarity | Role drift across co-chairs / website focus | Define decision boundaries |
| Delivery capacity | Multiple WPs running in parallel | Staged resourcing |
| Data integration | Mapping multiple datasets | Early technical alignment |
| Sustainability | Funding beyond 2027 | Forecast + early funding model |

| **#** | **Deliverable** | **Due** |
| --- | --- | --- |
| 1   | Community database established | Q1 2026 |
| 2   | Public interactive community map | Q2 2026 |
| 3   | Internal community management map | Q2 2026 |
| 4   | Community needs analysis report | Q2 2026 |
| 5   | UKTRE strategic role definition | Q2 2026 |
| 6   | Engagement strategy and roadmap | Q3 2026 |
| 7   | Engagement activities delivered | Jul 2026/Mar 2027 |
| 8   | Leadership pathway framework | Q4 2026 |
| 9   | Sustainable funding model | Q1 2027 |
| 10  | Funding forecast to April 2030 | Mar 2027 |

**ADD QUATERLY REPORT TO THE DELIVERABLE -**

**IS's thoughts**

The UK TRE Community is operating as an influential national community with a clear purpose and demonstrable impact. The proposed transition from a predominantly convening organisation towards a combined convening and coordination function is justified by both community need and ecosystem maturity.

The next phase should focus less on creating new activities and more on strengthening coordination, reducing fragmentation, improving sustainability, and developing future leadership capacity. Successful delivery of these priorities will place the UK TRE Community in a strong position to continue supporting the UK's Trusted Research Environment ecosystem through 2030 and beyond.
